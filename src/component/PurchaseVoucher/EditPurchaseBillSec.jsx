...
const EditPurchaseBillSec = ({ onChildDataChange,data}) => {
  ...
  const handleRemoveItem = useCallback(
    (index) => {
      ...
      // Safely handle add_tax value
      let additionalTaxRate = 0;
      if (typeof item.add_tax === 'string' && item.add_tax.includes('@')) {
        additionalTaxRate = parseFloat(item.add_tax.split('@')[0]) || 0;
      }
...
export default EditPurchaseBillSec;