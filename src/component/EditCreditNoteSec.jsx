...
const EditCreditNoteSec = ({ onChildDataChange,data}) => {
  ...
  useEffect(() => {
    ...
    if (data?.items) {
      ...
      const shippingCost = parseFloat(data?.credit_note?.shipping_cost) || 0;
      if (shippingCost > 0) {
      ...
      setState((prevState) => ({
        ...
        shippingCost: Number(data?.credit_note?.shipping_cost) || 0,
  ...
  useEffect(() => {
    ...
    const invoiceData = {
      ...
      total_gst: gstTotal,
      credit_note_items: state?.addedItems?.map((item) => ({
        id: item.id,
        item_id: item.item_id,
        quantity: item.quantity,
        unit_id: item.unit_id,
...
export default EditCreditNoteSec;
